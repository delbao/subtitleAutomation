...
def lrcwriter(filepath,subtitles,mode=0):
    ...
    for index,rec in enumerate(subtitles,1):
      ...
      if last_start!=0 and rec.start - last_start < 2000:
        last_finish = rec.finish
        duration = rec.finish - last_start
        text = rec.text
        if mode==1:
          ...
          for i in range(0,len(parts)):
            ...
            if lang!=strlang:
                if strlang == 'chs':
                    text_chs = text_chs + ' '.join(parts[i:])
                    text_eng = text_eng + ' '.join(parts[:i])
                else:
                    text_chs = text_chs + ' '.join(parts[:i])
                    text_eng = text_eng + ' '.join(parts[i:])
                break
        ...
        if last_start!=0:
          time = ms2time(last_start, 2)
          text = text_eng + '|' + text_chs
          text = text.replace('\r\n','')
          text = text.replace('\n','')
...
#        print text
        if mode==0:
          ...
          for i in range(0,len(parts)):
            ...
            if lang!=strlang:
                if strlang == 'chs':
                    text_chs = ''.join(parts[i:])
                    text_eng = ''.join(parts[:i])
                    print text_eng
                else:
                    text_chs = ''.join(parts[:i])
                    text_eng = ''.join(parts[i:])
